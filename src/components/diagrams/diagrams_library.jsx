...
interface DiagramShellProps {
  width?: string | number; // css width for the container
  ...
  className?: string;
  backgroundGrid?: boolean; // subtle grid background
}

export const DiagramShell: React.FC<DiagramShellProps> = ({
  width = "100%",
  height = "600px",
  minZoom = 0.25,
  maxZoom = 3,
        ...
        <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
          <defs>
            <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M 40 0 L 0 0 0 40" fill="none" stroke="var(--border)" strokeWidth="0.5" opacity="0.06" />
            </pattern>
          </defs>
...
// MindMap component
// -------------------------
export type MindMapNode = { id: string; label: React.ReactNode; x: number; y: number; color?: string };
...
// -------------------------
export type FlowNode = { id: string; label: React.ReactNode; x?: number; y?: number; width?: number; height?: number };
    ...
    <div className="relative w-full h-full">
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
        ...
        {edges.map((e, i) => {
          ...
          return <path key={i} d={path} strokeWidth={2} stroke="var(--border)" fill="none" markerEnd="url(#flow-arrow)" />;
        })}
      </svg>
      ...
      {positioned.map((n) => (
        <motion.div
          ...
          className="absolute"
          style={{ left: n.x, top: n.y, width: n.width, height: n.height, translate: "-50% -50%" }}
        >
...
// -------------------------
export type OrgNode = { id: string; label: React.ReactNode; parent?: string };
...
export const OrgChart: React.FC<OrgChartProps> = ({ nodes }) => {
  ...
  type LayoutNode = { id: string; depth: number; order: number };
          ...
          return (
            <g key={ln.id}>
              {children.map((cid) => {
                ...
                return (
                  <path key={cid} d={`M ${x} ${y + 28} L ${x} ${(y + cy) / 2} L ${cx} ${(y + cy) / 2} L ${cx} ${cy - 24}`} strokeWidth={2} stroke="var(--border)" fill="none" markerEnd="url(#org-arrow)" />
                );
              })}
            </g>
          );
...
export interface VennSet {
  id: string;
  label: string;
  size?: number; // optional weight
}

interface VennProps {
  sets: VennSet[]; // 1..3 sets
}

export const VennDiagram: React.FC<VennProps> = ({ sets }) => {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      <svg viewBox={`0 0 ${w} ${h}`} width={w} height={h} className="max-w-full">
        ...
        {s.map((set, i) => (
          <g key={set.id}>
            <circle cx={positions[i].x} cy={positions[i].y} r={r} fill={i === 0 ? "#FDE68A" : i === 1 ? "#A7F3D0" : "#BFDBFE"} fillOpacity={0.6} stroke="var(--border)" strokeWidth={1} filter="url(#soft)" />
            <text x={positions[i].x} y={positions[i].y - r - 12} textAnchor="middle" className="text-xs font-semibold fill-[var(--card-foreground)]" style={{ fontSize: 13 }}>
              {set.label}
            </text>
          </g>
        ))}
...
export const Timeline: React.FC<TimelineProps> = ({ items }) => {
  ...
  return (
    <div className="relative w-full h-full p-6">
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
        <line x1={40} x2={w - 40} y1={120} y2={120} strokeWidth={2} stroke="var(--border)" />
      </svg>
